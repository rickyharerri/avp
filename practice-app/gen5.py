...
def _b64(path):
    with open(path,"rb") as f: import base64; return base64.b64encode(f.read()).decode()
def img(s,x,y,w,h,href,clip):
    data=_b64(href)
    s.append(f'<image x="{x}" y="{y}" width="{w}" height="{h}" preserveAspectRatio="xMidYMid slice" xlink:href="data:image/jpeg;base64,{data}"/>')
...
def build():
    ...
    s.append(f'<rect x="0" y="{y}" width="{W}" height="{h}" fill="#140f08" fill-opacity="0.34"/>')
    play(s,W/2,y+h/2,46)
    ...
    s.append(f'<rect x="0" y="{ay}" width="{W}" height="{ah}" fill="#140f08" fill-opacity="0.5"/>')
    ...
    for fn,t,tag in cells:
        ...
        s.append(f'<rect x="{gx}" y="{gy}" width="{gw}" height="360" fill="#000" fill-opacity="0.12"/>')
